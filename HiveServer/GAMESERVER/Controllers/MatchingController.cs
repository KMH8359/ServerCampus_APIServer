...
public class Matching : ControllerBase
{
    ...
    public RedisConnection matchingDataRedisConn;
    private readonly string matchingDataRedisConnectionString;
    ...
    public Matching(ILogger<Matching> logger,  IMemoryDb memoryDb, HttpClient httpClient, IConfiguration configuration)
    {
        ...
        matchingDataRedisConnectionString = configuration["DbConfig:MatchingDataRedis"];

        RedisConfig config = new("default", matchingDataRedisConnectionString);
        matchingDataRedisConn = new RedisConnection(config);
    }
