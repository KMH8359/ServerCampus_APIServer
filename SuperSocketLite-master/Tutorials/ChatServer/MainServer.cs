...
namespace ChatServer
{
    public class MainServer : AppServer<ClientSession, EFBinaryRequestInfo>
    {
        ...
        {
            Room.NetSendFunc = this.SendData;   // 모든 Room의 SendFunc함수를 SendData 함수로 고정
            RoomMgr.CreateRooms();  // 생성된 Room은 계속해서 재활용된다.  -> 이용이 끝나도 버리지 않고 남겨둔다.
        ...
        {   // SuperSocket의 OnPacketReceived는 내부적으로 스레드 풀로 동작한다. -> 입력받은 버퍼의 크기, 최대 접속자 수 제한 등을 파악하고 내부적으로 풀의 크기를 결정한다.
