...
public class CheckUserAuthAndLoadUserData
{
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        using (StreamReader reader = new(context.Request.Body, Encoding.UTF8, true, 4096, true))
        {
            ...
            (bool isOk, UserAuthData userInfo) = await _memoryDb.GetUserAsync(email);
            if (isOk == false)
            {
                return;
            }
            ...
            context.Items[nameof(UserAuthData)] = userInfo;
        }
    ...
    private static async Task<bool> IsInvalidUserAuthTokenThenSendError(HttpContext context, UserAuthData userInfo, string authToken)
    {
